function getActivity_week(step, active_minutes, heart_minutes, heart_level) {
    ...
    var get_score = step_score + low_exercise_score +
            high_exercise_score + (heart_level * 10);

    return get_score
}

function calculateActivity({step, active_minutes, heart_minutes, heart_level}) {

    //weekly least levels of physical activity
    const MIN_WEEK_ACTIVITY = 5250;
    const MAX_WEEK_ACTIVETY = 15750;

    const totalScore = getActivity_week(
        step,
        active_minutes,
        heart_minutes,
        heart_level
